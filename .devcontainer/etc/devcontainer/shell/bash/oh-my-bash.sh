...
# This configures the Oh My Bash framework that enhances the bash shell with   #
# themes, plugins, and additional functionality                                #
#------------------------------------------------------------------------------#

# Set the path to the Oh My Bash installation directory
# Uses system-wide installation in /etc/oh-my-bash
export OSH="/etc/oh-my-bash"

# Theme configuration
# "agnoster" is a popular powerline-compatible theme with git status indicators
# and colorful prompt segments showing username, hostname, directory, etc.
OSH_THEME="agnoster"
...
function prompt_aws {
  local profile region icon

  icon="☁️ "  # AWS icon 

  if [[ -n "$AWS_PROFILE" ]]; then
    profile="$AWS_PROFILE"
    region=$(aws configure get region 2>/dev/null || echo "no-region")
    prompt_segment green black "$icon $profile@$region"
  else
    prompt_segment magenta black "$icon not connected"
  fi
}
